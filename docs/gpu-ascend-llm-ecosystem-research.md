...
> 数据说明：本报告数据综合自厂商官方技术报告、arXiv 论文、开源社区文档及行业媒体报道。涉及未发布硬件规格、价格、训练细节等数据存在预测或估算成分，请以官方信息为准。
...
> 注：910B 的 ~376 TFLOPS 为官方标称峰值，实际部署常见 256–320 TFLOPS；950PR/950DT、960/970 规格基于华为 2025 年公开路线图与行业媒体披露，截至 2026 年 6 月尚未全部量产或正式发布，实际参数以官方发布为准。
...
**关键数据**：DeepSeek-V4 在昇腾平台完成推理适配与训练验证，CANN 针对昇腾生态重写/优化大量算子；单卡 910B BF16 标称峰值约 376 TFLOPS，910C 约 780 TFLOPS。其在昇腾上的端到端训练效率与 H100 的差距尚无公开权威 benchmark，实际表现高度依赖 CANN 版本、并行策略与算子优化程度。
...
**关键意义**：LongCat-2.0 与 DeepSeek-V4 同日发布，形成独立验证——国产算力生态已能支撑前沿级万亿参数大模型训练。其中 DeepSeek-V4 明确在昇腾平台完成适配与推理验证；LongCat-2.0 的具体芯片型号与训练细节未官方披露。
...
> 注：单卡算力对比基于官方标称峰值；若按 910B 实际常见 256–320 TFLOPS 计算，相对 H100 的差距会更大。集群训练效率区间受软件优化、并行策略、网络拓扑影响显著。
...
- 2026 年 vLLM-Ascend 已支持 DeepSeek-V3/V3.2/R1、Qwen3、GLM5、Kimi-K2、MiniMax-m2 等主流模型。
...
- **对于本项目**：建议以 vLLM-Ascend 作为昇腾推理的标准入口，MindSpeed-LLM 作为训练入口，与当前昇腾生态趋势保持一致。
...
> 注：华为芯片通常以整机柜或服务器形式销售，单卡价为反推估算；910B/910C 价格受制裁、产能、批量影响波动较大，H100 在国内实际采购可能显著高于海外零售价。实际采购需通过华为或授权代理商询价。
